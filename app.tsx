// bb-plugin-productive — frontend entry.
//
// A near-clone of the Taskboard plugin's board UI, trimmed to one provider
// (Productive.io) and one credential (an org-wide API token). There is no
// source switching, no priority field, and every bb project maps to exactly
// one Productive project (board-settings.ts, contract.ts, server.ts already
// encode that). board-view.ts (pure helpers: grouping, lanes, filter
// options, collapse state) is reused as-is; this file is the React layer on
// top of it.
//
...
import {
  ...
  type FilterPreset,
  type ProductiveFolderOption,
  type ProductiveProjectOption,
  type ProductiveRpcContract,
  type ProductiveTaskListOption,
...
const THREAD_PANEL_ACTION_ID = 'productive-panel';
...
const FILTER_PRESENTATION: Record<WorkItemFilterField, FilterPresentation> = {
  ...
  status: { label: 'Status', icon: 'Workflow' },
  assignee: { label: 'Assignee', icon: 'UserRound' },
  folder: { label: 'Folder', icon: 'FolderOpen' },
  taskList: { label: 'Task list', icon: 'Folder' },
...
// Productive locators are numeric strings today, but this round-trips any
...
/**
 * "folder › task list" breadcrumb text, matching Productive's own
 * folder/list hierarchy. Returns null when the item has neither — text only,
 * never markup, since folder/task list names are untrusted API text.
 */
function folderBreadcrumb(item: Pick<WorkItem, 'folder' | 'taskList'>): string | null {
  const parts = [item.folder, item.taskList].filter(
...
/**
 * Productive serves attachment bytes from files.productive.io behind a
 ...
 * it in the user's browser where their Productive session is live.
 */
      ...
      <div className="space-y-1">
        ...
        <p className="max-w-md text-sm text-muted-foreground">
          {filtered
            ...
            : 'Use Manage to map this project to Productive, then refresh.'}
        </p>
      </div>
      {filtered ? (
...
const GROUP_BY_LABEL: Readonly<Record<LaneGrouping, string>> = {
  workflowStatus: 'Workflow status',
  taskList: 'Task list'
};

/** Kanban-only: switches lane grouping between workflow status and task list. */
function KanbanGroupByToggle({
  groupBy,
  onGroupByChange
}: {
  groupBy: LaneGrouping;
  onGroupByChange: (groupBy: LaneGrouping) => void;
}) {
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          ...
          aria-label={`Group lanes by ${GROUP_BY_LABEL[groupBy]}`}
        >
      ...
      <DropdownMenuContent align="start" className="min-w-44">
        ...
        {LANE_GROUPINGS.map(option => (
          <DropdownMenuItem key={option} onSelect={() => onGroupByChange(option)}>
            <span className="min-w-0 flex-1 truncate">{GROUP_BY_LABEL[option]}</span>
            {groupBy === option ? <Icon name="Check" className="size-3.5" /> : null}
          </DropdownMenuItem>
        ))}
      </DropdownMenuContent>
    </DropdownMenu>
...
function TrackerFilterBar({
  ...
  folderOptions: readonly FilterOption[];
  taskListOptions: readonly FilterOption[];
  ...
  const activeFacetCount = [
    ...
    enabledFilters.includes('folder') && filters.folders.length > 0,
    enabledFilters.includes('taskList') && filters.taskLists.length > 0,
  ...
  return (
    ...
    >
      ...
      />
      {filters.view === 'kanban' ? (
        <KanbanGroupByToggle groupBy={groupBy} onGroupByChange={onGroupByChange} />
      ) : null}
      <FilterPresetMenu
        presets={presets}
        error={presetsError}
        ...
        >
          ...
          {(
            [
              ...
              ['folder', folderOptions, filters.folders, 'folders'],
              ['taskList', taskListOptions, filters.taskLists, 'taskLists'],
 ...
 * project's mapped Productive project. Reused here rather than adding a
        ...
        <span className="pointer-events-none flex min-w-0 items-center gap-2 overflow-hidden">
          {folderBreadcrumb(item) ? (
            <span
              ...
              title={folderBreadcrumb(item) ?? undefined}
            >
              {folderBreadcrumb(item)}
            </span>
          ) : null}
...
function mergeDiscoveredStatuses(
  ...
  const merged = new Map(
    current.map(status => [workflowStatusLaneKey(status.name, status.stateCategory), status])
  );
  let changed = false;
  ...
  return (
    <div className="relative">
      <button
        ...
        aria-busy={pending}
        aria-label={`${item.key}: ${item.title}. Status ${item.status}.${assignee ? ` Assigned to ${assignee}.` : ''}${moveDisabled ? ' Workflow statuses are loading. Press Enter to open.' : ' Press Space to move, or Enter to open.'}`}
        data-state-category={item.stateCategory}
        data-status-tone={workflowStatusTone(item.status, item.stateCategory)}
      ...
      >
        ...
        {folderBreadcrumb(item) ? (
          <span className="mt-1 block truncate pl-[22px] text-2xs text-muted-foreground">
            {folderBreadcrumb(item)}
          </span>
        ) : null}
        {labels.length > 0 ? (
...
function KanbanBoard({
  ...
  items: readonly WorkItem[];
  preloadItem: WorkItem | null;
  statusOrder: readonly string[];
  groupBy: LaneGrouping;
  taskListOrder: readonly { id: string; name: string }[];
  onOpen: (item: WorkItem) => void;
  onMove: (item: WorkItem, option: WorkStatusOption) => Promise<void>;
  onMoveTaskList: (item: WorkItem, taskListId: string | null) => Promise<void>;
  canStartThread: boolean;
  startThreadPendingId: string | null;
  onStartThread: (item: WorkItem, environment?: 'project-default' | 'worktree') => void;
}) {
  const rpc = useRpc<ProductiveRpcContract>();
  const optionsRef = useRef(new Map<string, Promise<readonly WorkStatusOption[]>>());
  ...
  const [taskListPickup, setTaskListPickup] = useState<{
    item: WorkItem;
    targetLane: string | null;
    mode: 'pointer' | 'keyboard';
  } | null>(null);
  ...
  const [workflowReady, setWorkflowReady] = useState(preloadItem === null);
  const [announcement, setAnnouncement] = useState('');
  const [visibleMessage, setVisibleMessage] = useState<string | null>(null);
  const byTaskList = groupBy === 'taskList';
  const workflowLanes = useMemo(
    () => workflowStatusLanes(items, discovered, statusOrder),
    [discovered, items, statusOrder]
  );
  const taskListLaneList = useMemo(
    () => taskListLanes(items, taskListOrder),
    [items, taskListOrder]
  );
  const lanes = byTaskList ? taskListLaneList : workflowLanes;
  ...
  const loadOptions = useCallback(
    (item: WorkItem) => {
      ...
      const request = rpc
        .call('statusOptions', { projectId: item.bbProjectId, locator: item.locator })
  ...
  useEffect(() => {
    ...
    void loadOptions(preloadItem)
      .catch(() => [] as readonly WorkStatusOption[])
      .then(options => {
        if (cancelled) return;
  ...
  const beginPickup = useCallback(
    async (item: WorkItem, mode: 'pointer' | 'keyboard') => {
      ...
      try {
        ...
        const targetLane = workflowStatusLaneKey(targets[0]!.name, targets[0]!.stateCategory);
        setPickup(current =>
          current && kanbanItemId(current.item) === itemId
            ? { ...current, options, targetLane: current.targetLane ?? targetLane }
            : current
        );
        setAnnouncement(
  ...
  const optionForLane = useCallback(
    (laneKey: string) =>
      pickup?.options.find(
        option => !option.current && workflowStatusLaneKey(option.name, option.stateCategory) === laneKey
      ),
    [pickup]
  );

  const commitMove = useCallback(
    async (item: WorkItem, laneKey: string, knownOptions: readonly WorkStatusOption[] = []) => {
      if (pending) return;
      const itemId = kanbanItemId(item);
      let option = knownOptions.find(
        candidate => !candidate.current && workflowStatusLaneKey(candidate.name, candidate.stateCategory) === laneKey
      );
      if (!option) {
        try {
          const options = await loadOptions(item);
  ...
  const keyboardTargets = pickup?.options.filter(option => !option.current) ?? [];

  // Task-list lane pickup mirrors the workflow-status pickup above, but with
  // no server round trip: every lane but the item's current one is a valid
  // target (see `taskListMoveTargets`), so there is nothing to "discover".
  const beginTaskListPickup = useCallback(
    (item: WorkItem, mode: 'pointer' | 'keyboard') => {
      const targets = taskListMoveTargets(item, taskListLaneList);
      if (targets.length === 0) {
        const message = `${item.key} has no other task list to move to.`;
        setVisibleMessage(message);
        setAnnouncement(message);
        return;
      }
      setTaskListPickup({ item, targetLane: targets[0]!.key, mode });
      setAnnouncement(
        `${item.key} picked up. ${targets.length} task list ${targets.length === 1 ? 'target' : 'targets'} available. ${targets[0]!.name} selected.`
      );
    },
    [taskListLaneList]
  );

  const commitTaskListMove = useCallback(
    async (item: WorkItem, laneKey: string) => {
      if (pending) return;
      const lane = taskListLaneList.find(candidate => candidate.key === laneKey);
      if (!lane) {
        const message = `${item.key} cannot move to that task list.`;
        setTaskListPickup(null);
        setVisibleMessage(message);
        setAnnouncement(message);
        return;
      }
      ...
      setTaskListPickup(null);
      setVisibleMessage(null);
      setAnnouncement(`Moving ${item.key} to ${lane.name}`);
      try {
        await onMoveTaskList(item, lane.taskListId);
        setAnnouncement(`${item.key} moved to ${lane.name}`);
      } catch (error) {
        const message = describeError(error);
        setVisibleMessage(`${item.key} stayed in ${item.taskList ?? '(No task list)'}. ${message}`);
        setAnnouncement(`${item.key} move failed. ${message}`);
      } finally {
        setPending(current => (current === itemId ? null : current));
      }
    },
    [onMoveTaskList, pending, taskListLaneList]
  );

  const taskListKeyboardTargets = taskListPickup ? taskListMoveTargets(taskListPickup.item, taskListLaneList) : [];
    ...
    <div role="region" aria-label="Kanban board" className="tb-kanban-area h-full min-h-0 overflow-auto p-2">
      ...
      {lanes.length === 0 ? (
        <div className="flex h-full items-center justify-center text-sm text-muted-foreground">
          No statuses in the current results
        </div>
      ) : (
        <div dir="ltr" className="ml-0 mr-auto flex min-h-full min-w-max flex-row gap-2.5">
          {lanes.map(lane => {
            const columnItems = byTaskList
              ? items.filter(item => taskListLaneKey(item.taskListId) === lane.key)
              : items.filter(
                  item => workflowStatusLaneKey(item.status, item.stateCategory) === lane.key
                );
            const category = 'category' in lane ? lane.category : null;
            const option = byTaskList ? undefined : optionForLane(lane.key);
            const isTaskListTarget =
              byTaskList && taskListPickup
                ? taskListMoveTargets(taskListPickup.item, taskListLaneList).some(
                    target => target.key === lane.key
                  )
                : false;
            const dropState = byTaskList
              ? taskListPickup
                ? isTaskListTarget
                  ? taskListPickup.targetLane === lane.key
                    ? 'target'
                    : 'valid'
                  : 'invalid'
                : 'idle'
            ...
            return (
              <section
                ...
                aria-dropeffect={
                  byTaskList
                    ? taskListPickup && isTaskListTarget
                      ? 'move'
                      : 'none'
                ...
                data-state-category={category ?? undefined}
                data-status-tone={category ? workflowStatusTone(lane.name, category) : undefined}
                onDragOver={
                  byTaskList
                    ? event => {
                        if (!taskListPickup && !draggedItemRef.current) return;
                        event.preventDefault();
                        event.dataTransfer.dropEffect = 'move';
                        setTaskListPickup(current => (current ? { ...current, targetLane: lane.key } : current));
                      }
                    : event => {
                ...
                onDrop={
                  byTaskList
                    ? event => {
                        event.preventDefault();
                        const item = draggedItemRef.current ?? taskListPickup?.item;
                        draggedItemRef.current = null;
                        if (item) void commitTaskListMove(item, lane.key);
                      }
                    : event => {
                        event.preventDefault();
                        const item = draggedItemRef.current ?? pickup?.item;
                        draggedItemRef.current = null;
                        if (item) void commitMove(item, lane.key, pickup?.options);
                      }
                }
                ...
                <div className="min-h-20 flex-1 space-y-1.5 p-1.5 pt-1">
                  {columnItems.length > 0 ? (
                      ...
                      return (
                        <KanbanCard
                          ...
                          pickedUp={
                            byTaskList
                              ? taskListPickup
                                ? kanbanItemId(taskListPickup.item) === itemId
                                : false
                              : pickup
                          ...
                          moveDisabled={byTaskList ? false : !workflowReady}
                          onPrepare={() => {
                          ...
                          onDragStart={event => {
                            if (byTaskList) {
                              ...
                              beginTaskListPickup(item, 'pointer');
                              return;
                            }
                          ...
                          onDragEnd={() => {
                            ...
                            setTaskListPickup(current => (current?.mode === 'pointer' ? null : current));
                            window.setTimeout(() => {
                          ...
                          onKeyDown={event => {
                            if (byTaskList) {
                              const isThisTaskListPickup =
                                taskListPickup && kanbanItemId(taskListPickup.item) === itemId;
                              if (!isThisTaskListPickup && event.key === ' ') {
                                event.preventDefault();
                                beginTaskListPickup(item, 'keyboard');
                                return;
                              }
                              if (!isThisTaskListPickup) return;
                              if (event.key === 'Escape') {
                                event.preventDefault();
                                setTaskListPickup(null);
                              ...
                              if (event.key === 'ArrowLeft' || event.key === 'ArrowRight') {
                                event.preventDefault();
                                const currentIndex = taskListKeyboardTargets.findIndex(
                                  target => target.key === taskListPickup.targetLane
                                );
                                const direction = event.key === 'ArrowRight' ? 1 : -1;
                                const next =
                                  taskListKeyboardTargets[
                                    (currentIndex + direction + taskListKeyboardTargets.length) %
                                      taskListKeyboardTargets.length
                                  ];
                                if (!next) return;
                                setTaskListPickup(current =>
                                  current ? { ...current, targetLane: next.key } : current
                                );
                              ...
                              if (event.key === 'Enter' || event.key === ' ') {
                                event.preventDefault();
                                if (taskListPickup.targetLane) {
                                  void commitTaskListMove(taskListPickup.item, taskListPickup.targetLane);
                                }
                              }
                              return;
                            }
                            if (!workflowReady && event.key === ' ') {
                              event.preventDefault();
                              setAnnouncement('Workflow statuses are still loading');
                              return;
                            }
                            const isThisPickup = pickup && kanbanItemId(pickup.item) === itemId;
                            ...
                            if (event.key === 'ArrowLeft' || event.key === 'ArrowRight') {
                              ...
                              const currentIndex = keyboardTargets.findIndex(
                                target => workflowStatusLaneKey(target.name, target.stateCategory) === pickup.targetLane
                              );
                              const direction = event.key === 'ArrowRight' ? 1 : -1;
                              const next =
                                keyboardTargets[
                              ...
                              if (!next) return;
                              const targetLane = workflowStatusLaneKey(next.name, next.stateCategory);
                            ...
                            if (event.key === 'Enter' || event.key === ' ') {
                              ...
                              if (pickup.targetLane) {
                                void commitMove(pickup.item, pickup.targetLane, pickup.options);
                              }
                            }
                          }}
                          onOpen={() => {
                            ...
                            if (!pickup && !taskListPickup) onOpen(item);
                          }}
                          canStartThread={canStartThread}
                    ...
                    <p className="px-2 py-5 text-center text-xs text-muted-foreground">
                      {dropState === 'target' ? 'Drop to move here' : 'No tasks'}
                    </p>
                  )}
                </div>
              </section>
...
function TrackerBoard({
  ...
  // Workflow status is Productive's own kanban default; task list is the
  // opt-in layout that mirrors picking a folder and seeing its lists as
  // columns.
  ...
  const statusOrder = boardSettings?.statusOrder ?? DEFAULT_WORKFLOW_STATUS_ORDER;

  // Task-list lane order must follow `listProductiveTaskLists`, not whatever
  // order tasks happen to load in — only needed once the user opts into that
  // grouping, and only when the board is scoped to a Productive project.
  const [taskListOrder, setTaskListOrder] = useState<ProductiveTaskListOption[]>([]);
  const productiveProjectId = items?.find(item => item.productiveProjectId)?.productiveProjectId ?? null;
  useEffect(() => {
    if (groupBy !== 'taskList' || !productiveProjectId) return;
    let active = true;
    void rpc
      .call('listProductiveTaskLists', { productiveProjectId })
      .then(result => {
        if (active) setTaskListOrder(result.taskLists);
      })
      .catch(() => undefined);
    return () => {
      active = false;
    };
  }, [groupBy, productiveProjectId, rpc]);
  ...
  const folderOptions = useMemo(
    () => folderFilterOptions(items ?? [], filters.folders),
    [items, filters.folders]
  );
  const taskListOptions = useMemo(
    () => taskListFilterOptions(items ?? [], filters.taskLists),
    [items, filters.taskLists]
  );
  ...
  const applyFilters = (next: Partial<BoardFilterState>) => {
    setFilters(current => {
      ...
      if (next.folders)
        merged.folders = canonicalizeSelectedFilterOptions(next.folders, folderOptions);
      if (next.taskLists)
        merged.taskLists = canonicalizeSelectedFilterOptions(next.taskLists, taskListOptions);
      if (next.labels)
  ...
  const moveItemTaskList = useCallback(
    async (item: WorkItem, taskListId: string | null) => {
      const result = await rpc.call('updateItemTaskList', {
        projectId: item.bbProjectId,
        locator: item.locator,
        taskListId
      });
  ...
  if (error) {
    ...
    body = (
      <KanbanBoard
        items={filteredItems}
        preloadItem={items[0] ?? null}
        statusOrder={statusOrder}
        groupBy={groupBy}
        taskListOrder={taskListOrder}
        onOpen={onOpen}
        onMove={moveItem}
        onMoveTaskList={moveItemTaskList}
        canStartThread={canStartThread}
        ...
        <article className="mx-auto w-full min-w-0 max-w-[52rem] flex-1 px-5 pb-16 pt-7 @3xl:px-10 @3xl:pt-10">
          ...
          <dl className="tb-detail-meta mt-5 grid grid-cols-2 gap-x-4 gap-y-3 border-y py-4 @[45rem]:hidden">
            <div className="min-w-0">
              ...
              <dd className="truncate text-sm font-medium">{item.folder ?? 'None'}</dd>
            </div>
            <div className="min-w-0">
              <dt className="text-xs text-muted-foreground">Task list</dt>
              <dd className="truncate text-sm font-medium">{item.taskList ?? 'None'}</dd>
            </div>
            <div className="min-w-0">
          ...
          {item.attachments.length > 0 ? (
            <section className="mt-7">
              ...
              <p className="mt-2 text-xs text-muted-foreground">
                Attachments open in Productive in your browser — Productive doesn't allow this plugin
                to preview the files inline.
              </p>
            </section>
          ) : null}
...
interface CreatedTaskResult {
  item: WorkItem;
  warnings: string[];
  mention: { provider: 'productive-task'; id: string; label: string };
}
...
function CreateTaskDialog(props: CreateTaskDialogProps) {
  ...
  useEffect(() => {
    ...
    void rpc
      ...
      .then(result => {
        ...
        setTaskListId(result.metadata.defaultTaskListId);
      })
      .catch(() => {
  ...
  const create = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (!projectId || !context?.available || creating || metadataLoading || loadedConnectorRevision === null) {
      return;
    }
    setCreating(true);
    setCreateError(null);
  ...
  return (
    ...
    >
      <DialogContent className="sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle>
            {context ? `${context.projectName} · Productive · New task` : 'Prepare task'}
          </DialogTitle>
          <DialogDescription>
            {context === undefined
              ? 'Loading the Productive project configured for this BB project…'
              : !context.available
                ? 'Finish setting up Productive for this project.'
                : !assisted
                  ? `Create a task directly in the Productive project mapped to ${context.projectName}.`
                  : 'Review and edit your composer prompt before creating the task.'}
          </DialogDescription>
        </DialogHeader>
        ...
        {contextError ? (
          ...
          <div className="space-y-3 rounded-lg border border-border bg-card p-4">
            <p role="alert" className="text-sm text-muted-foreground">
              {context.message ?? 'Productive is not ready.'}
            </p>
            <Button
              type="button"
              size="sm"
              ...
              {metadataLoading ? (
                ...
                <>
                  ...
                  <IssuePropertySelect
                    ...
                    label="Task list"
                    value={taskListId}
                    options={metadata.taskListOptions}
...
function DirectCreateTaskAction({ projectId }: { projectId: string | null }) {
  ...
  const label = projectId ? 'Create a new Productive task' : 'Choose a BB project before creating a task';
  return (
    <>
      <Button
        type="button"
          ...
          <TooltipTrigger asChild>
            <span className="inline-flex">
              <Button
                ...
                aria-label="Create Productive task"
                ...
                onClick={() => {
                  ...
                  if (!hasPrompt) {
                    toast.info('Write a prompt first, then click the Productive ticket.');
                    composer.focus();
                    return;
                  }
 ...
 * (direct `saveConnection` RPC) and the `productive-connection` pending
 * interaction (server-triggered `bb.ui.requestInput`, submit/cancel props).
 * The token field never echoes a configured secret back — only whether one
 * is configured.
 */
function ConnectionForm({
  ...
  return (
    ...
    >
      <div className="grid gap-1.5">
        <label htmlFor={`${formId}-org`} className="text-xs font-semibold">
          Organization id
        </label>
        <Input
          id={`${formId}-org`}
      ...
      <div className="rounded-lg border border-border bg-card p-3">
        <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
          <label htmlFor={`${formId}-token`} className="text-xs font-semibold">
            API token
          </label>
          <CredentialStatus
            configured={tokenConfigured}
            hasDraft={tokenDraft.trim() !== ''}
            remove={removeToken}
          />
        </div>
        <Input
          ...
          spellCheck={false}
          value={tokenDraft}
          placeholder={tokenConfigured ? 'Enter to replace current token' : 'Enter your Productive API token'}
      ...
      <div className="grid gap-1.5">
        <label htmlFor={`${formId}-person`} className="text-xs font-semibold">
          Your person id (optional)
        </label>
        <Input
          id={`${formId}-person`}
          value={personId}
        ...
        />
        <p className="text-xs text-muted-foreground">
          Productive has no “who am I” endpoint, so this id is what powers “assigned to me”
          filtering and the assigned-to-me board option. Find it in Productive under your
          profile, or ask an admin.
        </p>
      </div>
  ...
  return (
    <div className="tb-settings-card space-y-4 rounded-xl border p-5">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <h3 className="text-sm font-semibold">Productive connection</h3>
        ...
        {connection.configured
          ? connection.available
            ? `Connected to organization ${connection.organizationId}${
                connection.viewerName ? ` as ${connection.viewerName}` : ''
              }.`
            : (connection.message ?? 'Connection is not available.')
          : 'Add an API token and organization id to connect this bb install to Productive. One connection serves every bb project.'}
      </p>
      <ConnectionForm
...
function ProjectMappingForm({ projectId }: { projectId: string }) {
  ...
  const debouncedQuery = useDebouncedValue(projectQuery, PROJECT_SEARCH_DEBOUNCE_MS);
  const [projectOptions, setProjectOptions] = useState<ProductiveProjectOption[]>([]);
  const [searchingProjects, setSearchingProjects] = useState(false);
  const [folderOptions, setFolderOptions] = useState<ProductiveFolderOption[]>([]);
  const [taskListOptions, setTaskListOptions] = useState<ProductiveTaskListOption[]>([]);
  ...
  // Productive orgs run to ~480 projects — debounce the search RPC so typing
  // does not fire a request per keystroke.
  useEffect(() => {
    let active = true;
    setSearchingProjects(true);
    void rpc
      .call('listProductiveProjects', { query: debouncedQuery })
      .then(result => {
        if (active) setProjectOptions(result.projects);
      })
      .catch(() => undefined)
      .finally(() => {
  ...
  useEffect(() => {
    if (!scope || scope.productiveProjectId === '') {
      setFolderOptions([]);
      return;
    }
    let active = true;
    void rpc
      .call('listProductiveFolders', { productiveProjectId: scope.productiveProjectId })
      .then(result => {
        if (active) setFolderOptions(result.folders);
      })
      .catch(() => undefined);
    return () => {
      active = false;
    };
  }, [rpc, scope?.productiveProjectId]);

  // The folder narrows which task lists (and therefore which board columns)
  // this bb project syncs — mirrors picking a folder in Productive's own UI.
  useEffect(() => {
    if (!scope || scope.productiveProjectId === '') {
      setTaskListOptions([]);
      return;
    ...
    let active = true;
    void rpc
      .call('listProductiveTaskLists', {
        productiveProjectId: scope.productiveProjectId,
        folderId: scope.folderId
      })
      .then(result => {
        if (active) setTaskListOptions(result.taskLists);
      })
      .catch(() => undefined);
    return () => {
      active = false;
    };
  }, [rpc, scope?.productiveProjectId, scope?.folderId]);
  ...
  if (!scope.connectionConfigured) {
    return (
      <div className="tb-settings-card rounded-xl border p-5 text-sm text-muted-foreground">
        Connect Productive above before mapping this project.
      </div>
    );
  }
  ...
  return (
    <div className="tb-settings-card space-y-4 rounded-xl border p-5">
          ...
          <DropdownMenuTrigger asChild>
            <Button type="button" variant="outline" className="w-full justify-start gap-1.5">
              ...
              <span className="min-w-0 flex-1 truncate text-left">
                {scope.productiveProjectName ?? 'Choose a Productive project'}
              </span>
          ...
          >
            <div className="sticky top-0 border-b border-border bg-popover p-2">
              <Input
                ...
                placeholder="Search Productive projects…"
                aria-label="Search Productive projects"
              />
            </div>
            <div className="p-1">
              {searchingProjects ? (
                ...
                projectOptions.map(option => (
                  ...
                  >
                    <span className="min-w-0 flex-1 truncate">{option.name}</span>
                    {scope.productiveProjectId === option.id ? (
                      <Icon name="Check" className="ml-auto size-3.5" />
                    ) : null}
                  </DropdownMenuItem>
                ))
      ...
      {scope.productiveProjectId !== '' ? (
        <div className="grid gap-1.5">
          <span className="text-xs font-semibold">Folder (optional)</span>
          <p className="text-xs text-muted-foreground">
            Narrows the board to one Productive folder — its task lists become the board's
            columns, just like picking a folder in Productive itself.
          </p>
          <Select
            value={scope.folderId || '__all__'}
            disabled={saving}
            onValueChange={value =>
              save({
                projectId,
                productiveProjectId: scope.productiveProjectId,
                folderId: value === '__all__' ? '' : value,
                // The task list filter is scoped to the previous folder; a
                // folder change resets it rather than silently keeping a
                // task list id that may not belong to the new folder.
                taskListId: '',
                assignedToMeOnly: scope.assignedToMeOnly,
                includeClosed: scope.includeClosed
              })
      ...
      {scope.productiveProjectId !== '' ? (
        <div className="grid gap-1.5">
          <span className="text-xs font-semibold">Task list filter (optional)</span>
          <Select
            value={scope.taskListId || '__all__'}
            disabled={saving}
            onValueChange={value =>
              save({
                projectId,
                productiveProjectId: scope.productiveProjectId,
                folderId: scope.folderId,
                taskListId: value === '__all__' ? '' : value,
                assignedToMeOnly: scope.assignedToMeOnly,
                includeClosed: scope.includeClosed
              })
            }
          >
            <SelectTrigger className="w-full">
              <SelectValue placeholder="All task lists" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="__all__">All task lists</SelectItem>
...
function TrackerSidebar({
  ...
  return (
    <aside
      aria-label="Productive navigation"
...
function ProductivePanel({ subPath }: PluginNavPanelProps) {
...
function ProductiveHeaderAction({ subPath }: PluginNavPanelProps) {
  const route = parseTrackerRoute(subPath);
  const { projectId: contextProjectId } = useBbContext();
  const navigate = useBbNavigate();
...
function ProductiveRightPanel({ projectId }: { projectId: string | null | undefined }) {
  const navigate = useBbNavigate();
...
function ProductiveThreadPanel({ threadId }: PluginThreadPanelProps) {
  const rpc = useRpc<ProductiveRpcContract>();
  ...
  useEffect(() => {
    ...
    void rpc
      ...
      .catch(nextError => {
        ...
        toast.error('Could not resolve this thread’s Productive project.', {
          description: describeError(nextError)
        });
      });
    return () => {
  ...
  return <ProductiveRightPanel projectId={projectId} />;
}

function ProductiveNewThreadPanel({ projectId }: PluginNewThreadPanelProps) {
  return <ProductiveRightPanel projectId={projectId} />;
}

function ProductiveThreadHeaderAction({ threadId }: PluginThreadHeaderActionProps) {
  const { openThreadPanel } = useBbNavigate();
  const autoOpenedThreadRef = useRef<string | null>(null);
  const openPanel = useCallback(
    (showError: boolean) => {
      const opened = openThreadPanel({ actionId: THREAD_PANEL_ACTION_ID, title: 'Productive' });
      if (!opened && showError) toast.error('Productive cannot open beside this thread.');
      return opened;
    },
    [openThreadPanel]
  );
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <Button
            ...
            size="icon"
            className="size-7"
            aria-label="Pin Productive on the right"
            onClick={() => {
        ...
        <TooltipContent>Pin Productive on the right</TooltipContent>
      </Tooltip>
    </TooltipProvider>
  );
}
...
// bb.ui.requestInput({ rendererId: 'productive-connection', payload })
// ---------------------------------------------------------------------------

function ProductiveConnectionInteraction({ interaction, submit, cancel }: PluginPendingInteractionProps) {
    ...
    <ConnectionForm
      ...
      onSubmit={mutation => {
        ...
        if (!validated.success) {
          setError('Enter a valid organization id and API token.');
          return;
        }
        setBusy(true);
        setError(null);
...
function ProductiveSettingsInfo() {
  const navigate = useBbNavigate();
  const { projectId } = useBbContext();
  return (
    <div className="space-y-3">
      <p className="text-sm text-muted-foreground">
        Connect the Productive API token once, then map each bb project to a Productive
        project, choose its filters, and set the default board layout.
      </p>
      <Button
        type="button"
        variant="outline"
      ...
      >
        <Icon name="Settings" className="size-4" />
        Open Productive project settings
      </Button>
    </div>
  );
}
...
export default definePluginApp(app => {
  ...
  app.slots.threadPanelAction({
    ...
    title: 'Productive',
    icon: 'Ticket',
    component: ProductiveThreadPanel,
    layout: 'flush'
  });
  app.slots.experimental_newThreadPanelAction({
    id: 'productive-new-thread-panel',
    title: 'Productive',
    icon: 'Ticket',
    component: ProductiveNewThreadPanel,
    layout: 'flush'
  });
  app.slots.experimental_threadHeaderAction({
    id: 'open-productive-panel',
    title: 'Productive',
    component: ProductiveThreadHeaderAction
  });
  app.slots.navPanel({
    id: 'productive',
    title: 'Productive',
    icon: 'Ticket',
    path: PANEL_PATH,
    component: ProductivePanel,
    headerContent: ProductiveHeaderAction
  });
  app.slots.settingsSection({
    id: 'connections',
    title: 'Productive',
    description: 'Connect Productive and map each project.',
    component: ProductiveSettingsInfo
  });
  app.slots.pendingInteraction({
    id: 'productive-connection',
    component: ProductiveConnectionInteraction
  });
  // There is no project-page slot in the SDK — the command palette is the
  // intended way to jump to the board from anywhere.
  app.slots.commandPaletteAction({
    id: 'open-board',
    title: 'Productive: Open board',
    run: context => {
      const navigate = useBbNavigate();
      navigate.toPluginPanel(PANEL_PATH, {
