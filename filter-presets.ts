...
//   browse-preferences.ts. This plugin has exactly one provider (Productive)
//   and is scoped to a single bb project, so that whole multi-source
//   preference store has no equivalent here. `filterPresetStateSchema` is
//   defined locally instead, matching the trimmed filter field set from
//   board-settings.ts: no `provider`/`source`/`priorities`, and
//   `taskLists` instead of `externalProjects`.
import { z } from 'zod';
...
const controlFreePresetStateInputSchema = z.unknown().superRefine(
  (input, context) => {
    ...
    for (const key of ['statuses', 'assignees', 'taskLists', 'labels']) {
      inspectBoundedStringArray(
        record,
        key,
...
const filterPresetStateShapeSchema = z
  .object({
    ...
    // Defaulted, not required: presets saved before folders existed are
    // already on disk, and a required field would make them unparsable —
    // which the store treats as a corrupt preset and drops.
    folders: uniqueValuesSchema.default([]),
    taskLists: uniqueValuesSchema,
