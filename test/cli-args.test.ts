...
describe('positionalArgs', () => {
  test('reads the locator whichever side of the flags it lands on', () => {
    ...
    const flagsFirst = ['show', '--project', 'proj_abc', '20075730'];
    const locatorFirst = ['show', '20075730', '--project', 'proj_abc'];
    expect(positionalArgs(flagsFirst)).toEqual(['show', '20075730']);
    expect(positionalArgs(locatorFirst)).toEqual(['show', '20075730']);
  });

  test('drops boolean flags without eating the next argument', () => {
    expect(positionalArgs(['show', '--json', '20075730'])).toEqual([
      'show',
      '20075730'
    ]);
    expect(
  ...
  test('keeps multi-word positional text together', () => {
    ...
    ).toEqual(['comment', '20075730', 'looks', 'good']);
  });

  test('a value flag with no value does not swallow the next flag', () => {
    expect(positionalArgs(['show', '--project', '--json', '20075730'])).toEqual([
      'show',
      '20075730'
    ]);
  });
  ...
  test('an unknown flag is not mistaken for a positional argument', () => {
    expect(positionalArgs(['show', '--nope', '20075730'])).toEqual([
      'show',
      '20075730'
    ]);
  });
...
describe('flagValue', () => {
  test('reads a value regardless of position', () => {
    expect(flagValue(['show', '20075730', '--project', 'proj_abc'], '--project'))
      .toBe('proj_abc');
    expect(flagValue(['show', '--project', 'proj_abc', '20075730'], '--project'))
      .toBe('proj_abc');
  });

  test('returns null for a missing flag, a trailing flag, or a flag followed by a flag', () => {
    expect(flagValue(['show', '20075730'], '--project')).toBeNull();
